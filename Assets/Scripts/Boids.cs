...
{
    /// <summary>
    /// A class that stores all the boid's related data.
    /// </summary>
    public static class BoidData
    {
        ...
        public const float FleeingPredatorPercentage = 50.0f;
        // Force applied to the boid when it encounters a border. 
        public const float BorderBouncingForce = 30.0f;
    }
    ...
    public class BoidsManager
    {
        ...
        public static BoidsManager Instance
        {
            get
            {
                if (m_instance == null)
                {
                    m_instance = new BoidsManager(BoidData.MaxBoidsCount);
                }
                return m_instance;
            }
        }
        ...
        private Vector2 BoidsGrouping(MonoBehaviour currentBoid)
        {
            ...
            for (int i = 0, c = Boids.Count; i < c; ++i)
            {
                b = Boids[i];
                if (b != currentBoid
                    && Vector2.SqrMagnitude(b.transform.position - currentBoid.transform.position) < BoidData.DetectionDistanceSquared)
                {
            ...
            return vel * BoidData.GroupingPercentage;
        }
        ...
        private Vector2 BoidsSeparation(MonoBehaviour currentBoid)
        {
            ...
            for (int i = 0, c = Boids.Count; i < c; ++i)
            {
                ...
                if (b != currentBoid)
                {
                    ...
                    if (distanceSquared < BoidData.DetectionDistanceSquared && distanceSquared < BoidData.SeparationDistanceSquared)
                    {
        ...
        private Vector2 BoidsCohesion(MonoBehaviour currentBoid)
        {
            ...
            return (perceivedVelocity - currentBoid.GetComponent<Rigidbody2D>().velocity) * 0.01f * BoidData.CohesionPercentage;
        }
        ...
        public void BoidsGSC(MonoBehaviour currentBoid, out Vector2 out_groupingAcc, out Vector2 out_separationAcc, out Vector2 out_cohesionAcc)
        {
            ...
            for (int i = 0, c = Boids.Count; i < c; ++i)
            {
                ...
                if (b != currentBoid
                    && (distanceSquared < BoidData.DetectionDistanceSquared))
                {
                    ...
                    if (distanceSquared < BoidData.SeparationDistanceSquared)
                    {
            ...
            out_groupingAcc = (out_groupingAcc / boidsCount - new Vector2(currentBoid.transform.position.x, currentBoid.transform.position.y)) * 0.01f * BoidData.GroupingPercentage;
        ...
        public static Vector2 BorderBouncing(MonoBehaviour currentBoid)
        {
            ...
            {
                v.X = BoidData.BorderBouncingForce;
            }
            ...
            {
                v.Y = BoidData.BorderBouncingForce;
            }
